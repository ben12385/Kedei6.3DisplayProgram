...
row = list()
for a in range(0, 480*320*2):
	row.append(-1)

currentImage = np.array(row).reshape(320,480,2)
...
while True:
	...
	newImage = np.array([ord(a) for a in imageRaw]).reshape(320,480,2)
	
	differenceImage = newImage - currentImage
	
	for y in range(0, 320):
		value = np.sum(differenceImage[y])
		if value != 0:
			if y < topLeftY:
				topLeftY = y
			if y > lowerRightY:
	...
	for x in range(0, 480):
		value = np.sum(differenceImage[:,x])
		if value != 0:
			if x < topLeftX:
				topLeftX = x
			if x > lowerRightX:
	...
	for y in range(topLeftY, lowerRightY+1):
		for x in range(topLeftX, lowerRightX+1): 		
			if newImage[y][x][1] == toSend[1] and newImage[y][x][0] == toSend[2]:
				GPIO.output(8, 1)
				GPIO.output(8, 0)
			else:
				toSend[1] = newImage[y][x][1]
				toSend[2] = newImage[y][x][0]
				sendPixel(toSend)
